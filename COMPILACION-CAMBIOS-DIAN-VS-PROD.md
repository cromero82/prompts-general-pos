...
**`metodo_pago`:** `descripcion_egreso`, `visible_pagos_egresos`, `visible_pago_tickets`, `monto`

**`establecimiento`:** `manejo_estricto_cuentas`

### 2.3 Scripts `00`–`26` (orden)
...
2. **consultar-rango:** Base desde `base_siguiente_efectivo` o inversión inicial; ventas con watermark `id > ultimo_historial_recibo_id` (evita recontar último ticket); movimientos con `id > ultimo_movimiento_origen_fondos_id`; excluye `SALIDA_EGRESO` / `ENTRADA_VENTA` de columna Movimientos.
...
Sobre todo: `CorteVenta`, `CorteVentaDetalle`, `OrigenFondos`, `MovimientoOrigenFondos`, `MotivoMovimiento`, `Egreso`, `MetodoPago`, `VentasTipo`.
...
1. Clonar/actualizar `dian-v2` + `dian-version`.
2. Tener dump o BD; si schema viejo → `apply-migrate-prod-to-dian-v2.sh`.
3. (Pruebas) reset transaccional opcional.
